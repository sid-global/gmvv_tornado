...
define("pg_pass", default="Master1029", help="User password for database", type=str)
define("pg_host", default="localhost", help="Database server", type=str)
define("pg_dbname", default="pdvsa_gmvv", help="Database server", type=str)
...
class ProductHandler(tornado.web.RequestHandler):
    ...
    def _get_products(self, product_ids = None):
        ...
        """
        if product_ids:
            self.cursor.execute("SELECT cor_modulo_sec, cor_modulo_id, cor_modulo_desc FROM cor_seg_modulo WHERE cor_modulo_id IN ('SEGURIDAD')", (product_ids, ))
        else:
            self.cursor.execute("SELECT cor_modulo_sec, cor_modulo_id, cor_modulo_desc FROM cor_seg_modulo")
...
class LoginHandler(tornado.web.RequestHandler):
    """
    Esta clase se encarga de manejar el Login de la Aplicacion. Esta modificada
	con el decorador @tornado.web.asynchronous con la intecion de hacer que el 
	llamado al metodo get sea asincrono, ya que esto es lo recomendado en caso 
	de que algun metodo del handler consuma mucho tiempo
    """
    @property
    def cursor(self):
        return self.application.cur
    ...
    def _login_callback(self):
        ...
        """
        cuando se genera la respuesta y esta es crossdomain, es decir, de dominios
        diferentes es necesario agregar el callback de la siguiente manera:
            callback_fcn({jdon_dict})
        la respuesta quedaria :
            jQuery150012353578762498318_1352483832941({"status": {"message": "", "id": "OK"}, "login": [{"pswd": "202cb962ac59075b964b07152d234b70", "login": "usuario", "id": 1}]})
    ...
    def get(self, *args, **kwargs):
        ...
        """
        Cuando se hacen llamados cross domain jquery agrega un metodo llamado callback
        que sebe ser agregado en la respuesta
        """
